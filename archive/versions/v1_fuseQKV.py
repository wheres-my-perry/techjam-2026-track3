...
and the fused-QKV implementation.
...
The CLI defaults are atol=0.002 and rtol=0.02 (2%).
"""
...
class FusedSelfAttention(BaselineSelfAttention):
    def __init__(self, d_model: int, num_heads: int) -> None:
...
class UserOptimizedTransformer(BaselineTransformer):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        for layer in self.layers:
            layer.attention = FusedSelfAttention(config.d_model, config.num_heads)
