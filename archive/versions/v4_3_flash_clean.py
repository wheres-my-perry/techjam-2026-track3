"""Standalone V4.3 Flash-first mixed-precision Transformer.
...
The optimized causal path assumes right padding: valid tokens form a prefix,
so causal attention already prevents valid queries from attending to padded
keys in the suffix. Non-causal inputs keep the key-padding mask.
"""
...
class UserOptimizedTransformer(nn.Module):
    """FP32 norm/residual with FP16 Flash-first attention, GEMMs, and GELU."""
    ...
    def _mixed_attention(
        ...
        if causal:
            # With right padding, valid tokens are a prefix. Causality already
            # blocks padded suffix keys from every valid query, so the key mask
            # is redundant and omitting it makes Flash Attention eligible.
            with sdpa_kernel(
